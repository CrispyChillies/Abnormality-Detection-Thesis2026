...
def check_requirements(file='/home/aaronpham5504/Coding/2nd-place-solution-for-VinBigData-Chest-X-ray-Abnormalities-Detection/part_sergey/yolo5/requirements.txt', exclude=()):
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45,
    ...
    """
    Returns:
        detections with shape: nx7 (x1, y1, x2, y2, conf, cls, branch_id)
    """

    nc = prediction.shape[2] - 6  # 5 + nc + 1(branch)
    ...
    for xi, x in enumerate(prediction):
        ...
        # Separate branch id before modifying x
        branch_id = x[:, -1:]  # shape [N,1]
        x = x[:, :-1]          # remove branch column temporarily
        ...
        if multi_label:
            ...
            x_out = torch.cat(
                (box[i], x[i, j + 5, None], j[:, None].float(), branch_id[i]),
                1
            )
        else:
            ...
            x_out = torch.cat(
                (box, conf, j.float(), branch_id),
                1
            )[mask]

        if classes is not None:
            x_out = x_out[(x_out[:, 5:6] ==
        ...
        if not n:
            ...
            x_out = x_out[x_out[:, 4].argsort(descending=True)[:max_nms]]

        c = x_out[:, 5:6] * (0 if agnostic else max_wh)
        boxes, scores = x_out[:, :4] + c, x_out[:, 4]
        ...
        if merge and (1 < n < 3E3):
            ...
            x_out[i, :4] = torch.mm(weights, x_out[:, :4]).float() / weights.sum(1, keepdim=True)
            if redundant:
                i = i[iou.sum(1) > 1]
